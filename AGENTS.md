...
- Providing reference material to the audit is allowed and is context, not an answer. Use `--scope-note` to inject the target's threat model and protocol-specific invariants (what must hold, what is trusted, which token assumptions apply). This raises P(find) without naming a bug; do not encode a known vulnerability or its location.
- Confirmability gates which targets are worth a campaign: prefer Solidity (Foundry), Rust (cargo), Go, JS/TS where the dig can execute-confirm. A hybrid Foundry+npm repo (dependencies from `node_modules`, e.g. OpenZeppelin) only builds after `npm install` precedes `forge build`; the toolchain warm-up handles this ordering automatically.
