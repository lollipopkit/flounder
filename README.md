...
5. **Verify candidates**: settle only the suspected or synthesized candidates that Dig did not already prove or refute. Standalone `verify` also accepts imported prior claims.
...
A cross-project Findings view tracks every project finding through submission states. Evaluation evidence is excluded by default and can be inspected with the explicit source filter; those rows link back to their Evaluation and do not expose disclosure tracking controls.
