...
- To make execution confirmation possible, point `--source` at the buildable project root (the directory holding the manifest and lockfile, e.g. `Cargo.toml`/`Cargo.lock`, `package.json`, `go.mod`, `foundry.toml`), not just a `src/` subdirectory. The workspace copy is recursive and manifests are surfaced to the model, so a project root is buildable; a bare `src/` is not. Confirm at the smallest native test granularity (a MockProver/unit/component test, not a full end-to-end production reproduction); the build phase (`purpose=build`: dependency resolution and compilation) is allowed to use a package registry, while the confirm/exploit run stays local.
